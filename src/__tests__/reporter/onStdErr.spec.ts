...
describe('onStdErr testing', () => {
  ...
  test('onStdErr call sendTestItemLog with DEFAULT_LOG_LEVELS.ERROR', () => {
    ...
    expect(reporter.sendTestItemLog).toHaveBeenCalledWith(
      { level: DEFAULT_LOG_LEVELS.ERROR, message: 'Some error log' },
      testCase,
    );
  });

  test('onStdErr call sendTestItemLog with DEFAULT_LOG_LEVELS.WARN', () => {
    jest.spyOn(reporter, 'sendTestItemLog');
    // @ts-ignore
    reporter.onStdErr('Some warn message', testCase);
    expect(reporter.sendTestItemLog).toHaveBeenCalledWith(
      { level: DEFAULT_LOG_LEVELS.WARN, message: 'Some warn message' },
      testCase,
    );
  });
});
