...
describe('ReportingApi', () => {
  beforeEach(() => jest.spyOn(utils, 'sendEventToReporter').mockClear());
  ...
  test('addAttributes should call sendEventToReporter with params', () => {
    ...
    expect(utils.sendEventToReporter).toHaveBeenCalledWith(event, attrs, suite);
  });
  ...
  test('setDescription should call sendEventToReporter with params', () => {
    ...
    expect(utils.sendEventToReporter).toHaveBeenCalledWith(event, description, suite);
  });
  ...
  test('setTestCaseId should call sendEventToReporter with params', () => {
    ...
    expect(utils.sendEventToReporter).toHaveBeenCalledWith(event, testCaseId, suite);
  });
  ...
  describe('Item status reporting', () => {
    for (const { method, status } of [
      ...
      test(`${method} should call sendEventToreporter with ${status} status`, () => {
        ...
        expect(utils.sendEventToReporter).toHaveBeenCalledWith(event, status, suite);
      });
    }
  ...
  describe('Launch status reporting', () => {
    for (const { method, status } of [
      ...
      test(`${method} should call sendEventToReporter with ${status} status`, () => {
        ...
        expect(utils.sendEventToReporter).toHaveBeenCalledWith(event, status);
      });
    }
  ...
  describe('Logs reporting', () => {
    ...
    for (const { method, level } of [
      ...
      test(`${method} should call ReporingApi.log with ${level} level`, () => {
        jest.spyOn(ReportingApi, 'log');
        // @ts-ignore
        ReportingApi[method]('message', file, suite);

        expect(ReportingApi.log).toHaveBeenCalledWith(level, 'message', file, suite);
      });
    }
    ...
    test('ReportingApi.log should call sendEventToReporter with params', () => {
      ...
      expect(utils.sendEventToReporter).toHaveBeenCalledWith(event, expectedData, suite);
    });
  });
  ...
  describe('Launch logs reporting', () => {
    ...
    for (const { method, level } of [
      ...
      test(`${method} should call ReporingApi.launchLog with ${level} level`, () => {
        jest.spyOn(ReportingApi, 'launchLog');
        // @ts-ignore
        ReportingApi[method]('message', file);

        expect(ReportingApi.launchLog).toHaveBeenCalledWith(level, 'message', file);
      });
    }
    ...
    test('ReportingApi.launchLog should call sendEventToReporter with params', () => {
      ...
      expect(utils.sendEventToReporter).toHaveBeenCalledWith(event, expectedData);
    });
  });
});
