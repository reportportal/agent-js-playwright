...
export class RPReporter implements Reporter {
  ...
  onStdErr(chunk: string | Buffer, test?: TestCase): void {
    if (test) {
      ...
      const level = isErrorLog(message) ? DEFAULT_LOG_LEVELS.ERROR : DEFAULT_LOG_LEVELS.WARN;
  ...
  sendLog(
    tempId: string,
    { level = DEFAULT_LOG_LEVELS.INFO, message = '', time = clientHelpers.now(), file }: LogRQ,
  ): void {
  ...
  async onTestEnd(test: TestCase, result: TestResult): Promise<void> {
    ...
    if (result.error) {
      ...
      this.sendLog(testItemId, {
        level: DEFAULT_LOG_LEVELS.ERROR,
        message: stacktrace,
      });
